...
public class TriadOptimizer {
    ...
    public List<Token> optimize() {
        ...
        adjustTransitions();
        rpn.removeIf(token -> (token.getType() == Lexem.EMPTY_LEXEME));
    ...
    private List<Triad> findTriads() {
        ...
        for (int i = 0; i < rpn.size(); ++i) {
            ...
            if (curToken.getType() == Lexem.FALSE_TRANSITION) {
                if (!inIfOrLoop) {
            ...
            if (curToken.getType() == Lexem.ASSIGN_OP) {
                if (!inIfOrLoop) {
                    ...
                    for (Triad tmpTriad : tmpTriads) {
                        try {
                            if ((tmpTriad.getOperation().getType() == Lexem.ASSIGN_OP) &&
                                    (!foundVars.contains(tmpTriad.getFirst().toString()))) {
                                triads.add(new DeleteIfExistsTriad((Variable)tmpTriad.getFirst()));
    ...
    private void buildTriads(List<Triad> triads, WrapInt pos) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (curType == Lexem.DIGIT ||
                    curType == Lexem.VAR) {
                args.push(buildArgument(curToken));
            }
            else if (curType == Lexem.ADDITION_OP || curType == Lexem.SUBTRACTION_OP ||
                    curType == Lexem.MULTIPLICATION_OP || curType == Lexem.DIVISION_OP ||
                    curType == Lexem.EQUAL_LOGICAL_OP || curType == Lexem.LESS_LOGICAL_OP ||
                    curType == Lexem.LESS_OR_EQUAL_LOGICAL_OP || curType == Lexem.MORE_LOGICAL_OP ||
                    curType == Lexem.MORE_OR_EQUAL_LOGICAL_OP) {
    ...
    private void optimizeTriads(List<Triad> triads) {
        for (int i = 0; i < triads.size(); ++i) {
            ...
            try {
                if (triad.getOperation().getType() == Lexem.ASSIGN_OP) {
                ...
                if (triad instanceof DeleteIfExistsTriad) {
    ...
    private Map<TriadArgument, Integer> buildNumbersOfDependencies(List<Triad> triads) {
        ...
        for (int i = 0; i < triads.size(); ++i) {
            ...
            try {
                if (curTriad.getOperation().getType() == Lexem.ASSIGN_OP) {
    ...
    private void adjustTransitions() {
        for (int i = 0; i < rpn.size(); ++i) {
            if (rpn.get(i).getType() == Lexem.UNCONDITIONAL_TRANSITION ||
                    rpn.get(i).getType() == Lexem.FALSE_TRANSITION) {
    ...
    private int getEmptyLexemCountBeforeIndex(int index) {
        int counter = 0;
        for (int i = 0; i < index; ++i) {
            if (rpn.get(i).getType() == Lexem.EMPTY_LEXEME) {
                ++counter;
            }
        }
