...
import stack_machine.StackMachine.Context;
import stack_machine.StackMachine.State;
import token.Token;
...
public class ReversePolishNotationInterpreter {
    ...
    public ReversePolishNotationInterpreter(List<Token> rpnList) {
    ...
    private void switchContextBack(Stack<Context> contextStack) {
        ...
        if (curContext.pos < curContext.rpnList.size() &&
                curContext.rpnList.get(curContext.pos).getType() == Lexem.RETURN_KW) {
            ...
            if (!curStack.empty()) {
                ...
                if (returnToken.getType() == Lexem.VAR) {
